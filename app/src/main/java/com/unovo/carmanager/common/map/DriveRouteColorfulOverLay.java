...
public class DriveRouteColorfulOverLay {
   ...
   */
  public DriveRouteColorfulOverLay(AMap amap, DrivePath path, LatLonPoint start, LatLonPoint end,
  ...
  private void addStartAndEndMarker() {
    startMarker = mAMap.addMarker((new MarkerOptions()).position(startPoint)
  ...
  private void colorWayUpdate(List<TMC> tmcSection) {
    ...
    //终点和规划路径之间连线
    addPolyLine(new PolylineOptions().add(mLatLngsOfPath.get(mLatLngsOfPath.size() - 1), endPoint)
        .setDottedLine(true));
    for (int i = 0; i < mLatLngsOfPath.size() && j < tmcSection.size(); i++) {
      ...
      if (segmentTotalDistance > segmentTrafficStatus.getDistance() + 1) {
        double toSegDis = distanceBetweenTwoPosition - (segmentTotalDistance
            - segmentTrafficStatus.getDistance());
  ...
  private void addThroughPointMarker() {
    if (this.throughPointList != null && this.throughPointList.size() > 0) {
      ...
      for (int i = 0; i < this.throughPointList.size(); i++) {
        ...
        if (latLonPoint != null) {
          throughPointMarkerList.add(mAMap.addMarker(new MarkerOptions().position(
