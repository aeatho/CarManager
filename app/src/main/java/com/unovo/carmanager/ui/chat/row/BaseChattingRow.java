 ...
 */
package com.unovo.carmanager.ui.chat.row;
...
import com.unovo.carmanager.ui.chat.holder.BaseHolder;
import com.unovo.carmanager.ui.chat.holder.ViewHolderTag;
...
public abstract class BaseChattingRow implements IChattingRow {
   ...
   */
  protected static void getMsgStateResId(int position, BaseHolder holder, ECMessage msg,
    ...
    if (msg != null && msg.getDirection() == ECMessage.Direction.SEND) {
      ...
      ViewHolderTag holderTag =
          ViewHolderTag.createTag(msg, ViewHolderTag.TagType.TAG_RESEND_MSG, position);
   ...
   * @return
   */
  public abstract boolean onCreateRowContextMenu(ContextMenu contextMenu, View targetView,
      ECMessage detail);
  ...
  protected abstract void buildChattingData(Context context, BaseHolder baseHolder,
      ECMessage detail, int position);

  @Override
  public void buildChattingBaseData(Context context, BaseHolder baseHolder, ECMessage detail,
      int position) {
  ...
  private void setContactPhotoClickListener(final Context context, BaseHolder baseHolder,
      final ECMessage detail) {
    ...
    //              contact.setNickname(contact.getContactid());
    //            }
    //            activity.mChattingFragment.getChattingFooter()
    //                .setLastText(
    //                    activity.mChattingFragment.getChattingFooter().getLastText() + "@" + contact
    //                        .getNickname() + (char) (8197));
